 ...
 <!-- 여기서부터임 페이지내용임 &************************************************************************************************* -->     
      <article class="container quote w-container">
         <p class="quote-text" data-ix="float-in-on-scroll">KKANGAZI는 다양한
            외모와 성격을 가진 고양이를 전문으로 분양하고 있습니다.</p></article>
      <body>
            <div class="container">
               ...
               enctype="multipart/form-data"
                        ...
                        <tr>
                           ...
                           <td colspan="5"><input type="text" placeholder="제목을 입력하세요."
                              name="dog_title" class="form-control" value="<%=board.getDog_title() %>" /></td>
                        </tr>
                   <tr>
                     <th>종류 : </th>
                              <td colspan="5">
                                 <select name="animaltype" class="form-control">
                             <% if(board.getAnimaltype().equals("cat")){ %> 
                     ...
                     <input type="text" placeholder="ex)푸들, 레브라도리트리버"
                              name="dc_type" class="form-control" value="<%=board.getDc_type() %>" /></td>
                          <th class="txt-h6 ls-03 weight-600 font-bm">성별</th>
                           <td class="txt-h6 ls-03 weight-600 font-bm" >
                                 암<input type = "radio"  name= "dc_sex"  class="" value="암컷" <%=(board.getDc_sex().equals("")?"checked":"") %>/>
                                 수<input type = "radio" name="dc_sex" class="m" value="수컷"  <%=(board.getDc_sex().equals("수컷")?"checked":"") %>/>
                        </tr>
                        <tr>
                           <th class="txt-h6 ls-03 weight-600 font-bm">나이</th>
                           <td class="txt-h6 ls-03 weight-600 font-bm" >
                           <input type="text" placeholder="ex)1,2년 1년 미만일 경우 개월"
                              name="dc_age" class="form-control" value="<%=board.getDc_age() %>" /></td>
                           <th class="txt-h6 ls-03 weight-600 font-bm">중성화 여부</th>
                           <td class="txt-h6 ls-03 weight-600 font-bm" >
                                 했음 <input type = "checkbox"  name= "dc_sex_neu" value="중성화 o" <%=(board.getDc_vaccinate().equals("중성화 o")?"checked":"") %>/>
                      </tr>
                         <tr>
                            <th class="txt-h6 ls-03 weight-600 font-bm">접종</th>
                           <td class="txt-h6 ls-03 weight-600 font-bm" >
                           <input type="text" placeholder="ex)예방접종여부"
                              name="dc_vaccinate" class="form-control" value="<%= board.getDc_vaccinate()%>" /></td>
                           <th class="txt-h6 ls-03 weight-600 font-bm">사이즈</th>
                           <td class="txt-h6 ls-03 weight-600 font-bm" >
                                 대<input type = "radio"  name= "dc_size"  class="" value="대형견" <%=(board.getDc_size().equals("대형견")?"checked":"") %>/>
                                 중<input type = "radio" name="dc_size" class="" value="중형견"  <%=(board.getDc_size().equals("중형견")?"checked":"") %> />
                                소<input type = "radio" name="dc_size" class="" value="소형견" <%=(board.getDc_size().equals("소형견")?"checked":"") %> />
                           </tr>
                        <tr>
                           <th>기타 사항:</th>
                           <td colspan="5"><textarea cols="10" placeholder="기타 특이사항 입력해주세요. "
                                 name="dc_others" class="form-control" ><%=board.getDc_others() %></textarea></td>
                        </tr>
                        <tr>
                           <th>내용:</th>
                           <td colspan="5"><textarea cols="10" placeholder="내용을 입력하세요."
                                 name="dog_comment" class="form-control"><%=board.getDog_comment() %></textarea></td>
                        </tr>
                        <tr>
                           <th>동물사진: 
...
function display() {
  document.getElementById('parah').innerHTML="파일을 업로드해주세요. ";
